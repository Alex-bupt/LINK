...
class DBpedia:
    ...
    def _process_entity_kg(self):
        ...
        # Filter out edges with relations 15, 21, 13, 18, 21
        filter_relations = torch.tensor([15, 21, 13, 18, 21], dtype=torch.long, device=self.edge_type.device)
        mask = ~torch.isin(self.edge_type, filter_relations)
        ...
        if self.debug:
            logger.debug(
                f'#edge: {self.edge_index.size(1)}, #relation: {self.num_relations}, '
                f'#entity: {self.num_entities}, #item: {len(self.item_ids)}'
            )
