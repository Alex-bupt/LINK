...
if __name__ == '__main__':
    ...
    lora_config = LoraConfig(
        ...
        inference_mode=False,  # 表示我们正在训练而不是推理
        r=64,  # 矩阵分解的秩
        lora_alpha=256,  # LoRA的超参数alpha
    ...
    if args.prompt_encoder is not None:
        load_path = args.prompt_encoder
        if not os.path.isfile(load_path):
            raise FileNotFoundError(f"The specified prompt encoder file does not exist or is not a file: {load_path}")
        print(f"Loading prompt encoder from: {load_path}")  # 添加调试信息
        prompt_encoder.load("/root/autodl-tmp/UniCRS-main/output/prompt_for_recommendation/best/")
    ...
    # 仅优化 model 的参数，不包括 prompt_encoder
